...
const App = () => {
  ...
  const [isBookmarkClosed, setIsBookmarkClosed] = useState(false);
  const [isMobile, setIsMobile] = useState(window.innerWidth <= 600);

  useEffect(() => {
    if (localStorage.getItem('bookmarkClosed') === '1') {
      setIsBookmarkClosed(true);
    }
  }, []);
  ...
  const handleBookmarkClose = () => {
    setIsBookmarkClosed(true);
    localStorage.setItem('bookmarkClosed', '1');
  };
  ...
  return (
    <>
      ...
      {!isBookmarkClosed && (
        <div className="bookmark-fixed" style={{ position: 'fixed', top: 16, right: 16, zIndex: 1000, flexDirection: 'column', alignItems: 'flex-end' }}>
          {/* Kapatma ikonu */}
          <button
